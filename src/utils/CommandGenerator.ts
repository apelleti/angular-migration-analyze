  ...
  generateFixCommands(results: any): string[] {
    ...
    try {
      ...
      if (requiredPeerDeps.length > 0) {
        const packages = requiredPeerDeps.map(
          (dep: any) => `${dep.package}@"${dep.requiredVersion}"`
        );
      ...
      if (outdatedAngularPackages.length > 0) {
        ...
        if (corePackage) {
          commands.push(
            `ng update @angular/cli@${corePackage.targetVersion} @angular/core@${corePackage.targetVersion}`
          );
        }
      }
