...
export class PerformanceAnalyzer extends BaseAnalyzer {
  async analyze(): Promise<Partial<AnalysisResult> & {
    ...
    const optimizationSuggestions = this.generateOptimizationSuggestions(bundleAnalysis, duplicatedDependencies);
    ...
    return {
      ...
      recommendations: optimizationSuggestions.map(opt => ({
        ...
        action: opt.action,
        command: opt.command,
        priority: opt.impact === 'high' ? 'high' : opt.impact === 'medium' ? 'medium' : 'low' as const
      }))
    };
  }
  ...
  private collectPackageVersions(dependencies: any, packageVersions: Map<string, Set<string>>): void {
